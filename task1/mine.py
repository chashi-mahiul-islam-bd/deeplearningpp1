...
# training data
train_data = pa1_dataloader.get_dataset(train = True)
train_loader = pa1_dataloader.load(train_data,
                                   batch_size=BATCH_SIZE,
                                   shuffle=True)

# testing data
test_data = pa1_dataloader.get_dataset(train = False)
test_loader = pa1_dataloader.load(test_data,
                                   batch_size=BATCH_SIZE,
                                   shuffle=True)
...
class CNN_MNIST(nn.Module):
    def __init__(self, fc_1_output, dropout_prob):
        ...
        self.conv1 = nn.Conv2d(1, 4, 1, padding=1)
        self.conv2 = nn.Conv2d(4, 8, 1, padding=1)
...
# =============================================================================
        ...
        if valid_loss <= valid_loss_min:
            print('Validation loss decreased ({:.6f} --> {:.6f}). Saving Model ...'.format(
                    valid_loss_min,
                    valid_loss))
            torch.save(model.state_dict(), 'cnn_model_SGD_mnist.pt')
...
model.load_state_dict(torch.load('cnn_model_SGD_mnist.pt'))
